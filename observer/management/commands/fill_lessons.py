...
class Command(BaseCommand):
    ...
    help = 'Import lessons sessions and related tags from a csv file. Csv file must be tab separated, otherwise specify the delimiter with the delmiter param otherwise'
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
          for i,row in enumerate(d):
            ...
            title = '%s / %s' % (row['title'], row['tag_session_code']) # title will be the course code indeed
            date = datetime.strptime(row['date'], '%m/%d/%Y')

            #print i, course_code, title
            course = WorkingDocument.objects.get(title=course_code)
            print session_type, date, row['date']
            for key in row:
              if key.startswith('tag_'):
                tag_value = row[key]
                tag_type = key.replace('tag_','').upper()
                #print tag_value, tag_type

            lesson, created = WorkingDocument.objects.get_or_create(
              title = title,
              type = session_type,
              parent = course,
            ...
            wod, created = WorkingDocument.objects.get(
              title=title,
            ...
            for key in row:
              if key.startswith('tag_'):
                tag_value = row[key]
                tag_type = key.replace('tag_','').upper()
                tag, created = Tag.objects.get_or_create(slug=slugify(tag_value), type=tag_type, defaults={
                  'name': tag_value
                })
                wod.tags.add(tag)
